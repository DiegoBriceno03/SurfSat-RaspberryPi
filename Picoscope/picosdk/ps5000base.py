...
from picosdk.picostatus import *
from picosdk.psutils import *
...
class PS5000Device(object):
	...
	def _set_info(self):
		...
		for tag in ("DRIVER_VERSION", "USB_VERSION", "HARDWARE_VERSION",
			...
			line = create_string_buffer(b"\0", MAX_INFO_LEN)
			required = c_int16()
	...
	def locate_buffer(self, channel, samples, segment, mode, downsample, index=None):
		...
		if self._handle <= 0:
			return pico_num("PICO_INVALID_HANDLE"), None
		""" validate channels """
		if channel not in (list(self._channel_set.keys()) + list(self._port_set.keys())):
...
def enumerate_units(loaded_lib):
	...
	if len(serials.value):
		return tuple(serials.value.split(b","))
	else:
		return ()
