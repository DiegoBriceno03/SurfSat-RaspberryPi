...
if __name__ == "__main__":
	...
	if status == ps.m.pico_num("PICO_OK"):
		...
		for channel in [ps.m.Channels.A, ps.m.Channels.B, ps.m.Channels.C, ps.m.Channels.D]:
			...
			if status == ps.m.pico_num("PICO_OK"):
				s, index = ps.locate_buffer(channel=channel, samples=samples, segment=0, mode=ps.m.RatioModes.raw, downsample=0)
			...
			plt.plot(time, data[channel], label='Channel ' + str(channel))
